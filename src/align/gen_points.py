...
def main(args):
    ...
    output_filename = os.path.join(args.output_dir, 'train_list.txt')
    error_filename = os.path.join(args.output_dir, 'error_list.txt')
    ...
    for fimage in dataset:
        if nrof_images_total%100==0:
            logger.debug("Processing %d, (%s)" % (nrof_images_total, nrof))
        nrof_images_total += 1
        ...
        try:
            ...
            # logger.debug('image_path {}, points {}'.format(image_path, points))
            ...
            # logger.debug('image_path {}, _landmark {}'.format(image_path, _landmark))
            if _landmark is None or len (_landmark) != 5:
                oline = '%s\t%d\n' % (target_file, int(fimage.classname))
                error_file.write(oline)
            else:
                oline = '%s\t%d' % (target_file, int(fimage.classname))
                for landmark in _landmark:
                    for coordinate in landmark:
                        oline += '\t{:.2f}'.format(coordinate)
                oline += '\n'
